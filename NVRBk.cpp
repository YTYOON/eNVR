...
CNVRBk::CNVRBk(void):m_bOpenDatabase(false),m_bATM(false),m_pMainframe(NULL)
{
	Initial();
}
...
CMainFrame* CNVRBk::GetMainFrame()
{ 
...
int CNVRBk::GetEmptyCameraIdx()
{
	...
	csSQL.Format(_T("SELECT ec_camera.camera_idx FROM ec_camera ORDER BY camera_idx"));
	try
	{
		CRecordset m_rs(&m_DB); 
		m_rs.Open(CRecordset::forwardOnly, csSQL.GetBuffer());
...
bool CNVRBk::UpdateGroupCameraGrpId(int OldGroupid, int NewGroupid, int CamId)
{
	...
	if (OldGroupid != NewGroupid)
	{
		...
		if (vcGrpCamObj.size() && vcGrpCamObj[0].cameragroupid == OldGroupid)
		{
			try
			{
				strSql.Format(_T("UPDATE  ec_camera_group_camera SET cameragroupid = %d , emap_x=0,emap_y=0 WHERE cameragroupcameraid = %d"), NewGroupid, vcGrpCamObj[0].cameragroupcameraid);
				m_DB.ExecuteSQL(strSql);
				bResult = true;
			}
			catch(CDBException * p)
...
CString CNVRBk::get_ONVIF_RTSP(const CString& url, const CString& csUser, const CString& csPWD)
{
...
CString CNVRBk::GetServicesName(int nService)
{
	CString csServiceName;

	if (GetOEMCompany() == Etro_Company)
	{
		switch(nService)
		{
		case STREAM:
	...
	else
	{
		...
		{
		case STREAM:
			csServiceName = CMS_STREAM_SERVICE_NAME;
			break;
		case RECORD:
			csServiceName = CMS_RECORDING_SERVICE_NAME;
			break;
		case EVENT:
			csServiceName = CMS_EVENT_SERVICE_NAME;
			break;
		case HEALTH:
			csServiceName = CMS_HEALTH_SERVICE_NAME;
			break;
		case REMOTE:
			csServiceName = CMS_REMOTE_SERVICE_NAME;
			break;
		}
	}
