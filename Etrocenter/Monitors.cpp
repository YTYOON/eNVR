...
CMonitors::CMonitors()
{
	...
	::EnumDisplayMonitors( NULL, NULL, (MONITORENUMPROC)AddMonitorsCallBack, (LPARAM)&addMonitor );
}
...
BOOL CALLBACK CMonitors::AddMonitorsCallBack( HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData )
{
	...
	int nMonitorCount(0);
	nMonitorCount = GetMonitorCount();

	if(pAddMonitor->currentIndex > pAddMonitor->pMonitors->GetSize()-1 || IsMonitor(hMonitor) == false) return false;

//#ifdef _DEBUG
//	CString csMsg;
//	csMsg.Format(_T("pAddMonitor->currentIndex:%d, hMonitor:%d, nMonitorCount:%d,pAddMonitor->pMonitors->GetSize():%d\r\n"),pAddMonitor->currentIndex,hMonitor,nMonitorCount,pAddMonitor->pMonitors->GetSize());
//	AfxMessageBox(csMsg);
//#endif
...
BOOL CMonitors::IsMonitor( const HMONITOR hMonitor )
{
	...
	::EnumDisplayMonitors( NULL, NULL, (MONITORENUMPROC)FindMatchingMonitorHandle, (LPARAM)&match );
