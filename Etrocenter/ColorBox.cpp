...
#define COLOR_TREEBKBUTTON RGB(154,181,210);//RGB(176,0,0);//RGB(232,89,5); //RGB(254,218,197);
...
CColorBox::CColorBox()
{
	m_color=COLOR_TREEBKBUTTON; //RGB(0,0,0);
	m_allowChange = TRUE;
	m_selected = FALSE;
	m_CustomColors = NULL;
}
...
//	ON_CONTROL_REFLECT_EX(BN_CLICKED, OnClickedEx)
	//}}AFX_MSG_MAP
//	ON_CONTROL_REFLECT(BN_CLICKED, &CColorBox::OnBnClicked)
//ON_CONTROL_REFLECT(BN_CLICKED, &CColorBox::OnBnClicked)
END_MESSAGE_MAP()
...
void CColorBox::SetIndex(const int o_index)
{
	m_index = o_index;
}

int CColorBox::GetIndex()
{
	return m_index;
}
...
//void CColorBox::OnBnClicked()
//{
//	//if (m_index<0) return;
//
//	CBarPtz* pBarPtz = (CBarPtz*)GetParent();
// 	
//
