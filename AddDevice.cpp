...
CAddDevice::CAddDevice(int nSelectGrpId, CWnd* pParent /*=NULL*/)
	...
	, m_bIP(FALSE)
	, m_strPassword(_T(""))
	, m_pbk(NULL), m_isExist(false),m_isAddSuccess(false),m_ipAddress(0)
{
...
BOOL CAddDevice::IsStation(CString& strBuffer)
{
	...
	return (BOOL) m_pbk->isStation(strBuffer, strAddress, strPort, m_strUserName, m_strPassword);
}
...
void CAddDevice::OnBnClickedAddButton()
{
	...
	{
		UpdateData(true);
		g_pThread = ::AfxBeginThread(AddDeviceThread, (LPVOID)this);
		GetDlgItem(IDOK)->EnableWindow(false);
	}
	else
	{
		DisplayError(SEARCHING_DEVICE);
	}
}
...
void CAddDevice::GetVideoFormat(CJason& jason, CString& video_format)
{
	...
	video_format.Format(_T("%d"), m_pbk->PackVideoFormat(0,(byte)m_DeviceType,0,byFirst));
}
...
void CAddDevice::GetPTZSupport(CString& ptz_support)
{
	...
	{
		CString str2;
		m_mapJason[_T("mcp")].GetValuebyKey(_T("System"),_T("mcp_system_speeddome"), str);
		m_mapJason[_T("mcp")].GetValuebyKey(_T("Lens"),_T("mcp_lens_motorized"), str2);

		if (str.Find(_T("yes")) != -1 || str2.Find(_T("yes")) != -1)
			ptz_support = _T("Y");
		else
			ptz_support = _T("N");
	}
...
void CAddDevice::GetStreamProtocol(int nStreamIdx, CString& stream_protocol)
{
	if (m_DeviceType == MODEL_1180)
	{
		...
		if (m_pbk->isLocalLan(strAddress, bAddSelf))
...
void CAddDevice::GetVideoStream(ec_Camera& camObj)
{
	...
	for(int nIdx = 0; nIdx < nCount; nIdx++)
	{
		...
		if (m_isONVIF)
		{
			...
			streamObj.stream_port = node.nAPPort;
		}
...
bool CAddDevice::isExistDevice(ec_Camera& camObj)
{
	...
	for(nIdx = 0; nIdx < nCount ; nIdx++)
	{
			...
			m_pbk->GetDevice(vccamObj[nIdx].video_format) == ONVIF)
		{
			...
			AfxMessageBox(_T("This device already exist!"));
			m_isExist = true;
			break;
		}
	}
...
{	
	...
	if (m_vcNodes.size() && !m_isStation)
	{
		...
		{
			GetONVIF_RTSP(m_vcNodes[0].stream_url, m_strUserName, m_strPassword, m_vcNodes[0]);
			bResult = true;
		}
	}
	else
...
void CAddDevice::GetONVIF_RTSP(const CString& url, const CString& csUser, const CString& csPWD, NODEITEM& node)
{
	CString str;
	str = m_pbk->get_ONVIF_RTSP(url, csUser, csPWD);
	GetONVIFStreamURL(str, node.stream_url);
	GetONVIFStreamPort(str,node.nAPPort);
}
	...
	if(m_isAddSuccess)
	{
		...
		if (!isExistDevice(camObj))
		{
			...
			if (vcGroup.size())
			{
				...
				groupCamObj.camera_idx = camObj.camera_idx;
				groupCamObj.numb_ = 0;
...
void CAddDevice::CloseThread()
{
	if (g_pThread)
	{
		HANDLE hThread = g_pThread->m_hThread;
