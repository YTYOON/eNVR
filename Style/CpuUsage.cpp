...
//		Q259390 in MSDN.
	...
	lnNewValue = PerfCounters.GetCounterValue(&pPerfData, dwObjectIndex, dwCpuUsageIndex, A2W(szInstance));
...
inline int CCpuUsage::GetCpuUsage(LPCTSTR pProcessName)
{
	static PLATFORM Platform = GetPlatform();
	...
	strcpy(szInstance,W2A(pProcessName));
...
inline int CCpuUsage::GetCpuUsage(DWORD dwProcessID)
{
	static PLATFORM Platform = GetPlatform();
