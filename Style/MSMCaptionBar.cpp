...
BOOL CMSMCaptionBar::CreateEx (CWnd* pParentWnd, UINT nID)
{
	...
	if (pParentWnd->IsKindOf (RUNTIME_CLASS (CMDIFrameWndEx)))
	{
    ...
    #define IDR_MAINFRAME                   128
	    ...
	    if (hResOem != NULL)
	    {
		    ...
		    HICON hIcon = (HICON)::LoadImage (hResOem, MAKEINTRESOURCE (nID), IMAGE_ICON, 
	    ...
	    CString strCaption;
        strCaption = CProductDefine::m_tzProductName;
	    ...
	    if (pManager != NULL)
	    {
		    if (pManager->LoadMSMCaptionButtonsIcons (_T("IDB_MSM_CAPTION_BTNS")))
		    {
			    ...
			    rt.bottom = pManager->GetMSMCaptionButtonsSize ().cy;
		    };

		    m_CaptionHeight = rt.bottom;
	    }
...
void CMSMCaptionBar::DoPaint (CDC* pDCPaint)
{
	...
	//CGS::GradientFillRect( pDC, CRect(rect.left, rect.top, rect.right, 22), RGB(100,100,100),RGB(0,0,0), GRADIENT_FILL_RECT_V);
	CGS::GradientFillRect( pDC, CRect(rect.left, rect.top, rect.right, 22), RGB(0,0,0), RGB(100,100,100), GRADIENT_FILL_RECT_V);
...
BOOL CMSMCaptionBar::bCheckSafeHwnd()
{
	BOOL bResult = FALSE;
	...
	{
		if ( m_BtnClose.GetSafeHwnd () != NULL &&
			 m_BtnMaximize.GetSafeHwnd () != NULL &&
...
BOOL CMSMCaptionBar::PreTranslateMessage (MSG* pMsg) 
{
    ...
    if (NULL == IsWindow(m_hWnd)) return false;
	...
	if (pParentWnd != NULL)
	{
        ...
        if (uiHit == HTCLOSE && pMsg->message == WM_LBUTTONUP)
        {
            pParentWnd->SendMessage (WM_CLOSE, NULL, NULL);
        }
	}
