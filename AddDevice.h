...
private:
	enum ErrorMsg{NOT_FOUND_DEVICE=0,SEARCHING_DEVICE};
...
private:
	...
	void GetONVIFStreamPort(const CString& str, WORD& nPort);
	void GetONVIFStreamURL(const CString& strRTSP, TCHAR *tzPath);
	bool CheckONVIF();
	void InsertDevice2DB(ec_Camera& camObj);
	void CallConfigDlg();
	void GetONVIF_RTSP(const CString& url, const CString& csUser, const CString& csPWD, NODEITEM& node);
